...
/**
 * GET /api/connect/google/callback?code=...&state=PHONE_NUMBER
 * Google redirects here after user authorizes. Exchange code for tokens and store for gmail.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[Connect Google] Token exchange failed:', error)
    return NextResponse.json(
  ...
  await prisma.appConnection.upsert({
    where: {
      userId_appName: { userId: user.id, appName: GMAIL_APP_NAME },
    },
    update: {
  ...
  try {
    await sendWhatsAppMessage(
      phoneNumber,
      '✅ Gmail connected successfully. You can now send, read, and manage email from here.'
    )
  } catch (_) {
